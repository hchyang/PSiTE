...
# Reivised by Bingxin Lu, 26/2/2018.
...
def processGZIP(gzipFile,paired):
    ...
    if paired:
       ...
       delD2=pickle.load(f)
       intD=pickle.load(f)
       gQualL=pickle.load(f)
       bQualL=pickle.load(f)
       iQualL=pickle.load(f)
       mates=pickle.load(f)
       rds=pickle.load(f)
       rdLenD=pickle.load(f)
       f.close()
