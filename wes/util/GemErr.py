...
def getRef(refFile):
    ...
    try:
        ...
        errlog.error('Cannot find reference file ' +
...
def updateM(ref, pos, seq, qual, cig, circ, mxNum, maxIndel, dir, readLen, excl):
    ...
    if cigH[1] == 'S':
        RposL -= cigH[0]
    # slice alignment out of ref, excluding masked sections.
    ...
    if dir == 'f':
        if RposR < refLen:
            # +1 to allow consideration of base AFTER last read base.
            refSlice += ref[RposR]
        else:
            if circ:
                # +1 for reads ending at last reference base (circular).
                refSlice += ref[0]
            else:
                # +1 for reads ending at last reference base (linear)
                refSlice += 'N'
    elif dir == 'r':
        if pos - 2 > 0:
        ...
        # so when we increment bpos it does in the right direction
        bPos = refLen - bPos - len(refSlice)
                ...
                if (c + cc) < minK:
                    ...
                    if (c + cc) < minK:
                        for i in range(readLen + 1):
                            tot = np.apply_over_axes(np.sum, matrix[mxNum][i], [1, 2, 3, 4])[
                                d1][0][0][0][0][5]
                            matrix[mxNum][i][d1][d2][d3][d4][d5][5] = tot
                            inserts = {}
                            key = str(i) + '.' + str(d1) + '.' + str(d2) + \
                                '.' + str(d3) + '.' + str(d4) + '.' + str(d5)
                            for ik in ikeys:
                            ...
                            if dels != [0] * maxIndel:
                                delD[mxNum][key] = dels
                            for n in range(5):
                                val = np.apply_over_axes(np.sum, matrix[mxNum][i], [1, 2, 3, 4])[
                                    d1][0][0][0][0][n]
                                matrix[mxNum][i][d1][d2][d3][d4][d5][n] = val
                    else:
                        for i in range(readLen + 1):
                            tot = np.apply_over_axes(np.sum, matrix[mxNum][i], [2, 3, 4])[
                                d1][d2][0][0][0][5]
                            matrix[mxNum][i][d1][d2][d3][d4][d5][5] = tot
                            inserts = {}
                            key = str(i) + '.' + str(d1) + '.' + str(d2) + \
                            ...
                            if dels != [0] * maxIndel:
                                delD[mxNum][key] = dels
                            for n in range(5):
                                val = np.apply_over_axes(np.sum, matrix[mxNum][i], [2, 3, 4])[
                                    d1][d2][0][0][0][n]
                                matrix[mxNum][i][d1][d2][d3][d4][d5][n] = val
                else:
                    for i in range(readLen + 1):
                        tot = np.apply_over_axes(np.sum, matrix[mxNum][i], [2, 3])[
                            d1][d2][0][0][d5][5]
                        matrix[mxNum][i][d1][d2][d3][d4][d5][5] = tot
                        inserts = {}
                        key = str(i) + '.' + str(d1) + '.' + str(d2) + \
                        ...
                        if dels != [0] * maxIndel:
                            delD[mxNum][key] = dels
                        for n in range(5):
                            val = np.apply_over_axes(np.sum, matrix[mxNum][i], [2, 3])[
                                d1][d2][0][0][d5][n]
                            matrix[mxNum][i][d1][d2][d3][d4][d5][n] = val
            else:
                for i in range(readLen + 1):
                    tot = np.apply_over_axes(np.sum, matrix[mxNum][i], [3])[
                        d1][d2][d3][0][d5][5]
                    matrix[mxNum][i][d1][d2][d3][d4][d5][5] = tot
                    inserts = {}
                    key = str(i) + '.' + str(d1) + '.' + str(d2) + \
                    ...
                    if dels != [0] * maxIndel:
                        delD[mxNum][key] = dels
                    for n in range(5):
                        val = np.apply_over_axes(np.sum, matrix[mxNum][i], [3])[
                            d1][d2][d3][0][d5][n]
                        matrix[mxNum][i][d1][d2][d3][d4][d5][n] = val
...
def lowCov(readLen, mxNum):
    inds = [0, 1, 2, 3, 4]
    tot = np.apply_over_axes(np.sum, matrix[mxNum], [0, 1, 2, 3, 4, 5])[
        0][0][0][0][0][0][5]
                    ...
                    for m in inds:
                        for n in inds:
                            if matrix[mxNum][i][j][k][l][m][n][5] < 20:
                                for o in inds:
                                    matrix[mxNum][i][j][k][l][m][n][o] = int(
                                        avg * matrix[mxNum][i][j][k][l][m][n][5])
...
def mkMxSingle(readLen, ref, samFile, name, skip, circular, maxIndel, excl, minK):
    ...
    matrix = [np.zeros([readLen + 1, 5, 5, 5, 5, 5, 6],
...
def mkMxPaired(readLen, ref, samFile, name, skip, circular, maxIndel, excl, minK):
    ...
    inds = {'A': 0, 'T': 1, 'G': 2, 'C': 3, 'N': 4,
            'a': 0, 't': 1, 'g': 2, 'c': 3, 'n': 4}
    global matrix, insD, delD, gQualL, bQualL, iQualL
    matrix = [None, np.zeros([readLen + 1, 5, 5, 5, 5, 5, 6], dtype='int32'),
              np.zeros([readLen + 1, 5, 5, 5, 5, 5, 6], dtype='int32')]
    insD = [None, {}, {}]
    delD = [None, {}, {}]
    intD = {}
    rds = [0, 0]
    ...
    errlog.info(str(float(mates[0]) / float(rds[0])) +
                '% first reads in pair with bad mates.')
    errlog.info(str(float(mates[1]) / float(rds[1])) +
                '% second reads in pair with bad mates.')
...
def main(argv):
    ...
    if paired:
        ...
        mkMxSingle(readLen, reference, samfile, name,
                   skip, circular, maxIndel, excl, minK)
